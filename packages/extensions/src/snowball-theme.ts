...
export const SNOWBALL_THEMES = {
  'snowball-default': {
    id: 'snowball-default',
    name: 'Snowball Default',
...
interface SnowballThemeOptions extends Record<string, unknown> {
  enabled?: boolean
  themeId?: string
}

export const SnowballThemeExtension = createExtension({
  name: 'snowballTheme',
  type: 'extension',
  defaultOptions: {
    enabled: true,
    themeId: 'snowball-default',
  } as SnowballThemeOptions,
  ...
  onCreate(ctx) {
    ...
    const options = ctx.storage as SnowballThemeOptions
    const themeId = options.themeId || 'snowball-default'
    const theme = SNOWBALL_THEMES[themeId as keyof typeof SNOWBALL_THEMES]

    if (!theme) {
      console.warn(`[SnowballThemeExtension] Theme "${themeId}" not found`)
      return
    }
